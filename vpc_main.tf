...
resource "aws_route_table" "pub_gtw" {
  count  = var.enable_igw == true ? 1 : 0
  vpc_id = aws_vpc.main.id
  ...
  tags = merge(local.tags, tomap({ "Name" : "pub-gtw-rt" }))
...
resource "aws_route_table" "priv_gtw" {
  ...
  tags = merge(local.tags, tomap({ "Name" : "priv-gtw-rt" }))
}


// ROUTE TABLE ASSOCIATION FOR PUBLIC SUBNETS
resource "aws_route_table_association" "public" {
  count          = length([for k, v in aws_subnet.main : k if lookup(v.tags, "subnet_type", "") == "public" && length(aws_route_table.pub_gtw) > 0])
  subnet_id      = element([for k, v in aws_subnet.main : v.id if lookup(v.tags, "subnet_type", "") == "public"], count.index)
  route_table_id = aws_route_table.pub_gtw[0].id
}

// ROUTE TABLE ASSOCIATION FOR PRIVATE SUBNETS
resource "aws_route_table_association" "private" {
  count          = length([for k, v in aws_subnet.main : k if lookup(v.tags, "subnet_type", "") == "private" && length(aws_route_table.priv_gtw) > 0])
  subnet_id      = element([for k, v in aws_subnet.main : v.id if lookup(v.tags, "subnet_type", "") == "private" && length(aws_route_table.priv_gtw) > 0], count.index)
  route_table_id = aws_route_table.priv_gtw[0].id
}
